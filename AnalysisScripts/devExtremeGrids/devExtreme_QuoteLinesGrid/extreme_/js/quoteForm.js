...
async function form_onload(executionContext) {
  ...
  if (formType !== FORM_NEW) {
    retryAttempt(() =>
      setClientApiContextForWebResource(formContext, "WebResource_quoteLines")
    );
  ...
  if (formType === FORM_NEW) {
    if (formContext.getAttribute("effectivefrom").getValue() === null) {
      ...
      var defaultQuoteValidDays = await readConfigurationValue(
        "defaultQuoteValidDays"
      );
      var newEffectiveTo = addDays(
        new Date(),
        parseInt(defaultQuoteValidDays, 10)
      );
      formContext.getAttribute("effectiveto").setValue(newEffectiveTo);
    }
    if (
      formContext.getAttribute("extreme_deliverymethod").getValue() === null &&
      formContext.getAttribute("extreme_paymentterms").getValue() === null
    ) {
      populateAccountDefaults();
    }
  } else {
    const createdOnDate = getCreatedOnDate(
    ...
    if (
      createdOnDate >
      new Date(formContext.getAttribute("effectivefrom").getValue())
    ) {
    ...
    formContext.getAttribute("effectivefrom").addOnChange(async () => {
      const effectiveFromDate = new Date(
        formContext.getAttribute("effectivefrom").getValue()
      );
      ...
      if (effectiveFromDate < createdOnDate) {
        ...
        Xrm.Utility.alertDialog(
          "Effective From date cannot be earlier than the Created On date."
        );
        return;
      }
  ...
  if (
    formContext.getAttribute("extreme_placeofpublishing").getValue() === null
  ) {
    var publishingLocation = await readConfigurationValue(
      "QuotePublishingLocation"
    );
    formContext
      .getAttribute("extreme_placeofpublishing")
      .setValue(publishingLocation);
  }
  ...
  formContext.getAttribute("statecode").addOnChange(async () => {
    ...
    if (
      quoteLinesControl &&
      quoteLinesControl.getObject() &&
      quoteLinesControl.getObject().contentWindow
    ) {
      await quoteLinesControl
        .getObject()
    ...
    formContext.getAttribute(attribute).addOnChange(async () => {
      ...
      req.open(
        ...
        Xrm.Utility.getGlobalContext().getClientUrl() +
          `/api/data/v9.2/quotes(${formContext.data.entity.getId().slice(1, -1)})`,
        false
      );
  ...
  async function populateAccountDefaults() {
    if (formContext.getAttribute("customerid").getValue() !== null) {
      ...
      var account = await Xrm.WebApi.retrieveRecord(
        "account",
        `${accountId}`,
        "?$select=_extreme_deliverymethod_value,_extreme_paymentterms_value"
      ).then(
      ...
      var deliveryMethodLookup = [
        {
          ...
          name: account[
            "_extreme_deliverymethod_value@OData.Community.Display.V1.FormattedValue"
          ],
          entityType:
            account[
              "_extreme_deliverymethod_value@Microsoft.Dynamics.CRM.lookuplogicalname"
            ],
        },
      ];
      ...
      if (account["_extreme_deliverymethod_value"] !== null)
        formContext
          .getAttribute("extreme_deliverymethod")
          .setValue(deliveryMethodLookup);

      var paymentTermsLookup = [
        {
          id: account["_extreme_paymentterms_value"],
          name: account[
            "_extreme_paymentterms_value@OData.Community.Display.V1.FormattedValue"
          ],
          entityType:
            account[
              "_extreme_paymentterms_value@Microsoft.Dynamics.CRM.lookuplogicalname"
            ],
        },
      ];
      ...
      if (account["_extreme_paymentterms_value"] !== null)
        formContext
          .getAttribute("extreme_paymentterms")
          .setValue(paymentTermsLookup);
    }
  }

  async function readConfigurationValue(key) {
    // eslint-disable-next-line no-undef
    var value = await Xrm.WebApi.retrieveMultipleRecords(
      "extreme_configuration",
      `?$select=extreme_value&$filter=extreme_key eq '${key}'&$top=1`
    ).then(
      ...
      wrControl
        ...
        .then(function (contentWindow) {
          try {
            ...
            console.log(e);
            retryAttempt(() =>
              setClientApiContextForWebResource(formContext, webResourceName)
            );
          }
        })
  ...
  function getCreatedOnDate(quoteId) {
    ...
    req.open(
      "GET",
      Xrm.Utility.getGlobalContext().getClientUrl() +
        `/api/data/v9.2/quotes(${quoteId})?$select=createdon`,
      false
    );
    req.setRequestHeader("OData-MaxVersion", "4.0");
    req.setRequestHeader("OData-Version", "4.0");
