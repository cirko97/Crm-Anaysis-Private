var employees = [{
  ...
  function masterGridColumnResized() {
    var detailContainers = grid.element().find('.internal-grid');
    ...
    if (detailContainers.length) {
      for (var j = 0; j < detailContainers.length; j++) {
        ...
        for (var i = 0; i < grid.columnCount(); i++) {
          detailGridInstance.columnOption(i, 'width', grid.columnOption(i, 'width'));
          detailGridInstance.columnOption(i, 'visibleWidth', grid.columnOption(i, 'visibleWidth'));
        }
  ...
  var grid = $("#gridContainer").dxDataGrid({
    ...
    masterDetail: {
      ...
      template: function(container, options) {
        ...
        var detailGrid = $("<div>").css("margin-left", "-30px")
